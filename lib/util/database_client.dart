...
class DatabaseHelper {
  ...
  initDb() async {
    ...
    String path = join(documentDirectory.path, "wap_db.db");
...
//insertion
  Future<int> saveItem(WapItem item) async {
...
//
  Future<WapItem> getItem(int id) async {
    var dbClient = await db;
    var result = await dbClient.rawQuery("SELECT * FROM $tableName WHERE id = $id");
    if (result.length == 0) return null;
    return new WapItem.fromMap(result.first);
  }
...
//  }
  ...
  Future<int> updateItem(WapItem item) async {
    var dbClient = await db;
